...
### Fixture shorthand expansion

These are readable normative examples, not canonical hash vectors. Unless a field is written explicitly, every `Node X (...)` declaration expands to the complete `ONTOLOGY.md` envelope as follows:

- `id`, `kind`, `scopeId`, and `disclosure` are the values printed in the fixture;
- `body` is `{"fixtureNode":"<id>"}`;
- `createdAt` is `t0`;
- `createdBy` is `fixture-author`;
- `provenance` is `[]` for a `source` and `["fixture:<fixture-number>"]` otherwise;
- `relationships` is `[]` at node admission; separately admitted fixture edges populate the graph projection.

Any fixture that overrides one of these defaults prints the override. Placeholder values such as `hashL1` and `hashR1` are visibly non-canonical until Issue #5 freezes canonical bytes and hashes.
...
5. **competing current harvests**:
   ...
   - *Expected Result*: Both nodes and edges admit successfully. Both harvests remain current without forcing consensus. Candidate membership steps sort by edge type, edge ID, direction, then destination object ID, so `e1` precedes `e2`; returned harvest nodes are deduplicated and sorted by node ID. (Defends Invariant 3).
...
7. **positive cross-scope bridge**:
   - *Inputs*:
     ...
     - Node Y (`id`: nY, `kind`: inference, `scopeId`: pubScope, `disclosure`: public)
     - Request Req1 (`id`: req1, `requester`: humanX, `purpose`: 'explicitly_declared_audit_request', `destinationScopeId`: pubScope, `declaredAt`: t0, `validFrom`: t0, `validUntil`: futureDate, `status`: open) *(Independently declared request)*
     ...
     - Receipt R1 (`receiptId`: r1, `receiptType`: RevelationReceipt, `issuedAt`: t1, `issuer`: humanX, `subject`: e1, `inputs`: { `sourceScopeId`: privScope }, `outputs`: { `destinationScopeId`: pubScope, `purpose`: 'explicitly_declared_audit_request', `requestRef`: req1 }, `authorityRef`: lease1, `policyRefs`: [public], `previousReceiptRefs`: [], `canonicalHash`: hashR1)
     - Edge E1 (`id`: e1, `type`: derived_from, `from`: nY, `to`: nX, `assertedBy`: humanX, `createdAt`: t1, `scopeId`: pubScope, `basis`: r1, `disclosure`: public).
   - *Expected Result*: E1 is admitted. Validation resolves `r1.outputs.requestRef` to exactly `Req1`, then matches requester, receipt issuer, edge actor, purpose, destination scope, open status, and validity. It also guards and resolves `basis` and `authorityRef` before dereferencing either receipt. Because `derived_from` pulls information from `nX` in `privScope` to `nY` in `pubScope`, the bridge is purpose-bound without silent exposure (Defends Invariant 9).
